...
router.get('/list', auth, async (req, res) => {
	try {
		...
		const products = await Product.find({ branch: req.user.id })
		res.json(products)
	} catch (err) {
		console.error(err)
		res.status(500).send({ msg: 'Server Error' })
...
router.post(
	...
	[
		auth,
		[
			check('description', 'Description is empty')
				.not()
				.isEmpty(),
			check('name', 'Name is empty')
				.not()
				.isEmpty(),
			check('rate', 'Rate is empty')
				.not()
				.isEmpty()
		]
	],
	async (req, res) => {
		...
		const { description, name, quantity, rate, type, unit } = req.body
